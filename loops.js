...
//Question 22 Print digits number from right to left
let digit = 4567; //Set number to use
while (digit > 0) { // Condition to get last digit
    let lastNum = digit % 10; // Creates a temp storage
    console.log(lastNum);  //Prints last number
    digit = Math.floor(digit / 10); // Continues until condition is met
}
//Alternate way to do question 22 using a String
let digit2 = "4567";
let num3 = digit2.length - 1;
while (num3 >= 0) {
    console.log(digit2[num3])
    num3--;
}
//Shorter way for ques 22
let digit3 = "4567"
for (let i = digit3.length - 1; i >= 0;  i--) {
    console.log(digit3[i]);
}

//Question 23 - Keep prompting the user until they enter the word stop
// Works in JavaScript but not terminal
// let input = "";
// while (input != "stop") {
//   input = prompt("Please enter something (Enter 'stop' to end.)");
// }

//Question 24 Print all Prime numbers between 1 and 50
let i = 2;
console.log(1);
while (i <= 50) { //Firt loop to go through numbers
    let prime = true; //Loop will continue as long as it's true
    let divisor = 2; //The number that we divide the number by
    

    while (divisor < i && prime){ //This loop is to determine if the number is a prime number
        if(i % divisor === 0){ //This tells me that the number is a prime number
            prime = false; //Loop will will stop if it is false 
        }
        divisor++;
    }
    if(prime){
        console.log(i); //Print if number is a Prime number
    }
    i++;
}

//Question 25 Use a while loop to calculate the power of a number and print the result (3^6=729)
let base = 3;
let exponent = 6;
let result = 1;
let counter = 1;
